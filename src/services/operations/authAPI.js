...
export function signUp(
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      toast.error(msg)
      // Stay on verify-email — do NOT navigate back to signup
    }
    dispatch(setLoading(false))
    toast.dismiss(toastId)
  }
...
export function login(email, password, navigate) {
  return async (dispatch) => {
    ...
    try {
      ...
      toast.dismiss(toastId) // dismiss before navigating
      navigate("/dashboard/my-profile")
    } catch (error) {
      toast.dismiss(toastId)
      // Show the actual server error message (e.g. "User is not registered")
