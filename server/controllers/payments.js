  ...
  for (const courseId of courses) {
    try {
      ...
      await createNotification({
        recipient: userId,
        type: "enrollment",
        title: `Enrolled: ${enrolledCourse.courseName}`,
        message: `You have successfully enrolled in "${enrolledCourse.courseName}". Start learning now!`,
        link: `/dashboard/enrolled-courses`,
        metadata: { courseId: enrolledCourse._id }
      })
    } catch (error) {
      console.log(error)
...
// Enroll in a FREE course (price = 0) without payment
exports.enrollFree = async (req, res) => {
  try {
    ...
    // Enroll
    await Course.findByIdAndUpdate(courseId, { $push: { studentsEnroled: userId } })
    const courseProgress = await CourseProgress.create({ courseID: courseId, userId, completedVideos: [] })
    await User.findByIdAndUpdate(userId, { $push: { courses: courseId, courseProgress: courseProgress._id } })

    const updatedUser = await User.findById(userId)
    await mailSender(
      updatedUser.email,
      `Successfully Enrolled in ${course.courseName}`,
      require("../mail/templates/courseEnrollmentEmail").courseEnrollmentEmail(
        course.courseName,
